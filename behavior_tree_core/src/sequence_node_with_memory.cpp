...
{
    current_child_ = 0;//initialize the current running child
}
...
BT::ReturnStatus BT::SequenceNodeWithMemory::Tick()
{
    ...
    DEBUG_STDOUT(get_name() << " ticked, memory counter: "<< current_child_);
    ...
    while (current_child_ < N_of_children_)
    {
        if (children_nodes_[current_child_]->get_type() == BT::ACTION_NODE)
        {
           ...
           // DEBUG_STDOUT(get_name() << "It is an action " << children_nodes_[current_child_]->get_name());
            ...
            child_i_status_ = children_nodes_[current_child_]->get_status();
            DEBUG_STDOUT(get_name() << " It is an action " << children_nodes_[current_child_]->get_name() << " with status: " << child_i_status_);
            ...
            if (child_i_status_ != BT::RUNNING)
            {
                if(child_i_status_ == BT::IDLE)
                {
                    ...
                    DEBUG_STDOUT(get_name() << "NEEDS TO TICK " << children_nodes_[current_child_]->get_name());
                    children_nodes_[current_child_]->tick_engine.Tick();
                }
                ...
                {
                // 1.2 -  If the  action status is not success, return the status
                    DEBUG_STDOUT("the status of: " << get_name() << " becomes " << child_i_status_);

                    set_status(child_i_status_);
                    return child_i_status_;
                }
                else if(current_child_ != N_of_children_ - 1)
                {
                ...
                }else if(current_child_ == N_of_children_ - 1)
                {
                    ...
                    {
                        //1.4 if it the last child and it has returned SUCCESS or FAILURE, reset the memory
                        current_child_ = 0;
                    }
                    set_status(child_i_status_);
                    return child_i_status_;
...
//                }
        ...
        else
        {
            ...
            child_i_status_ = children_nodes_[current_child_]->Tick();


            if(child_i_status_ != BT::SUCCESS)
            {
                // 2.1 -  If the  non-action status is not success, return the status
                set_status(child_i_status_);
            ...
            {
                // 2.2 -  If the  non-action status is success, continue to the next child (if any, hence if(current_child_ != N_of_children_ - 1) ) in the for loop (if any).
                std::cout << get_name() << " increasing the memory counter" << std::endl;

                current_child_ = current_child_ +1;
                std::cout << get_name() << "NEW memory counter: " << current_child_<< std::endl;
...

            {
                if(child_i_status_ == BT::SUCCESS || child_i_status_ == BT::FAILURE)
                {
                    ...
                    //3 if it the last child and it has returned SUCCESS or FAILURE, reset the memory
                    current_child_ = 0;
                }
                set_status(child_i_status_);
                return child_i_status_;
